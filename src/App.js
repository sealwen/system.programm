	...
	return (
		<div className="container">
			<div className="row justify-content-between">
				...
				<button className="btn btn-primary col-3 mt-5 mb-5" onClick={() => setShow(true)}>Начать</button>
				<button className="btn btn-primary col-3 mt-5 mb-5" onClick={add}>Добавить</button>
				<button className="btn btn-primary col-3 mt-5 mb-5" onClick={remove}>Удалить</button>
			</div>
			<h2 className="text-center">P</h2>
			<input className="form-control mb-5" value={n} onChange={e => setN(n =>
				isNaN(parseInt(e.target.value)) ? 0 : parseInt(e.target.value))} />
			<Thread arr={tk} update={setTk} />
			{show &&
				<>
					<Table2 values={First(myArr, tk, n, false, dev)} title="Очередь" />
					<Table2 values={First(myArr, tk, n, true, dev)} title="Приоритетное очередь" />
					<Table2 values={Second(myArr, tk, n)} title="Циклическое" />
					<Table2 values={Third(myArr, tk, n)} title="Минимум" />
				</>
			}
